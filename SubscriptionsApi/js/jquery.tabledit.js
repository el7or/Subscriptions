    ...
    $.fn.Tabledit = function (options) {
            ...
            columns: {
                ...
                editable: function () {
                    for (var i = 0; i < settings.columns.editable.length; i++) {
                        ...
                        $("input[name='BookedDate']").datepicker({
                            ...
                            onSelect: function (dateText) {
                                ...
                                var selectedSpecialist = $(this).closest('tr').find('td:eq(1) select option:selected').val();
                                var reservations = AllCheckinsObj.filter(function (checkinObj) {
                                    return new Date(checkinObj.ReservationDate).setHours(0) == new Date(sDate(dateText)).setHours(0)
                                        && checkinObj.Specialist == selectedSpecialist
                                });
                                $(this).closest('tr').find('td:eq(4)').text("Current Count: "+reservations.length);
                            }
                        });
                        $("input[name='BookedTime']").timepicker({ timeFormat: 'H:i'});
                ...
                toolbar: function () {
                    if (settings.editButton || settings.deleteButton) {
                        ...
                        $table.find('tr:gt(0)').append('<td class="tdActions" style="white-space: nowrap; width: 1%;padding-left: 8px !important;">' + toolbar + '</td>');
                    }
                }
            }
        };
        ...
        var Mode = {
            view: function (td) {
                ...
                if (settings.editButton) {
                    ...
                    $tr.find('button.tabledit-restore-button').hide();
                    $tr.find('.btn-group.btn-group-sm').show();
                }
            },
            edit: function (td) {
                Delete.reset(td);
                ...
                if (settings.editButton) {
                    ...
                    $tr.find('button.tabledit-restore-button').show();
                    $tr.find('.btn-group.btn-group-sm').hide();
                }
            }
        };
        ...
        var Delete = {
            reset: function (td) {
                ...
                // Hide restore button.
                $table.find('.tabledit-restore-button').hide();
                //Show save button & delete button.
                $table.find('.btn-group.btn-group-sm').show();
            },
            submit: function (td) {
                Delete.reset(td);
                ...
                //Show save button & delete button.
                $(td).find('.btn-group.btn-group-sm').show();
            },
            confirm: function (td) {
                ...
                // Show restore button.
                $(td).find('.tabledit-restore-button').show();
                //Hide save button & delete button.
                $(td).find('.btn-group.btn-group-sm').hide();
            },
            restore: function (td) {
                // Enable identifier hidden input.
                $(td).parent('tr').find('input.tabledit-identifier').attr('disabled', false);
                ...
                // Hide confirm button.
                $(td).find('.tabledit-confirm-button').hide();
                //Show save button & delete button.
                $(td).find('.btn-group.btn-group-sm').show();
                // Remove "active" class in delete button.
                $table.find('.tabledit-delete-button').removeClass('active').blur();
                $(td).closest('tr').find('td:eq(4)').text("");
            }
        };
